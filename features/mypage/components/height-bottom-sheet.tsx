...
const HEIGHTS = Array.from({ length: MAX_HEIGHT - MIN_HEIGHT + 1 }, (_, i) => i + MIN_HEIGHT);
const DATA = [...Array(PADDING_COUNT).fill(null), ...HEIGHTS, ...Array(PADDING_COUNT).fill(null)];

function parseHeight(value: string): number {
  const num = parseInt(value.replace(/[^0-9]/g, ''), 10);
  return isNaN(num) ? 170 : Math.min(MAX_HEIGHT, Math.max(MIN_HEIGHT, num));
}
...
export function HeightBottomSheet({ visible, initialValue, onClose, onSave }: Props) {
  const insets = useSafeAreaInsets();
  const flatListRef = useRef<FlatList>(null);
  const initialHeight = parseHeight(initialValue);
  const [selected, setSelected] = useState(initialHeight);

  // 스크롤 중 re-render 없이 현재 인덱스 추적
  const currentIndexRef = useRef(HEIGHTS.indexOf(initialHeight));

  useEffect(() => {
    if (!visible) return;
    const parsed = parseHeight(initialValue);
    const idx = HEIGHTS.indexOf(parsed);
  ...
  const isChanged = selected !== initialHeight;

  const handleScroll = (e: any) => {
    const y = e.nativeEvent.contentOffset.y;
    currentIndexRef.current = Math.max(
      0,
      Math.min(HEIGHTS.length - 1, Math.round(y / ITEM_HEIGHT)),
    );
  };
